...
if (!subgraph_options.includes(subgraph)) {
    throw new Error(`Subgraph '${subgraph}' does not match a valid subgraph: [${subgraph_options.join(',')}]`)
}

const chain_options = fs.readdirSync(path.resolve(ROOT_PATH, 'config', subgraph))
    .map(path.parse)
    .map(({name}) => name)
if (chain_options.length === 0) {
    throw new Error(`No chains detected for subgraph '${subgraph}'. Add chain config files via './config/${subgraph}/{chain}.json'`)
}
if (!chain_options.includes(chain)) {
    throw new Error(`Chain '${chain}' does not match a valid chain for '${subgraph}': [${chain_options.join(',')}]`)
}

// Config
const config = JSON.parse(fs.readFileSync(path.resolve(ROOT_PATH, 'config', subgraph, `${chain}.json`), 'utf-8'))

// Identify mapping templates
const source_directory = path.resolve(ROOT_PATH, 'src', subgraph)
const template_fileNames = fs.readdirSync(source_directory)
    .filter(fileName => fileName.includes('.template.'))

// Apply mapping templates
for (const template_fileName of template_fileNames) {
    const templateInput = fs.readFileSync(path.resolve(source_directory, template_fileName), 'utf-8')
    const templateOutputData = Mustache.render(templateInput, config)

    const templateOutputFilePath = ['schema.template.graphql', 'subgraph.template.yaml'].includes(template_fileName)
        ? path.resolve(ROOT_PATH, template_fileName.replace('.template.', '.'))
        : path.resolve(source_directory, template_fileName.replace('.template.', '.output.'))
